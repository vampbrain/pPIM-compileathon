...
class PIMInstructionGenerator:
    ...
    def generate_instructions(self, matrix_sizes):
        ...
        # Use smaller block for very large matrices to reduce register pressure
        # For small matrices, larger blocks reduce overhead
        ...
        # Depends on the memory layout - if C matrix fits in a continuous region,
        # we might be able to initialize it in a single operation
    ...
    def _initialize_c_matrix(self, c_rows, c_cols):
        ...
        # Check if matrix C is stored contiguously and can be initialized in fewer operations
        memory_map = self.memory_mapper.get_memory_map()
        if 'C' in memory_map:
            c_info = memory_map['C']
            start_row = c_info['row_offset']
            size = c_rows * c_cols
            
            # If C matrix is in a single subarray and fits within a small number of rows,
            # we can initialize it more efficiently
            if size <= 512:  # Assuming we can initialize a range of rows efficiently
                # Zero out each row of C (could combine multiple elements per row if possible)
                for i in range(c_rows):
                    for j in range(0, c_cols, 4):  # Assume we can write 4 elements at once
                        # Calculate row address for this block
                ...
                # Initialize individual elements (less efficient, but works for any layout)
                for i in range(0, c_rows, 4):  # Process in chunks of 4 rows for efficiency
                    for j in range(0, c_cols, 4):  # Process in chunks of 4 columns
                        # Initialize this 4x4 block (or smaller at edges)
    ...
    def _process_block(self, i_start, i_end, j_start, j_end, k_start, k_end):
        ...
        # Implement MAC operations for this block
        # Optimize by reusing values when possible
        
        # Track which elements are currently loaded to avoid redundant loads
        ...
        for i in range(i_start, i_end):
            for j in range(j_start, j_end):
                ...
                for k in range(k_start, k_end):
                    # Only load A[i][k] if not already loaded
                    ...
                    # Only load B[k][j] if not already loaded
                ...
                # Clear tracking sets when they get too large to prevent memory bloat
    ...
    def add_instruction(self, instr_type, pointer, read, write, row_addr):
        ...
        # 8-bit operation segment (2-bit type + 6-bit pointer)
        op_segment = self.instruction_types[instr_type] + format(pointer, '06b')
        
        # 10-bit memory access segment (read bit + write bit + 8-bit row address)
        # FIXED: Changed to 9-bit row address as per spec
        mem_segment = str(read) + str(write) + format(min(row_addr, 511), '09b')  # Limit to 9 bits
        
        # 6 bits reserved for future expansion
        reserved = '000000'
        
        # Combine segments to form 24-bit instruction
...
def add_instruction(self, instr_type, pointer, read, write, row_addr):
    ...
    # 10-bit memory access segment (read bit + write bit + 8-bit row address)
    # Using 9-bit row address as per spec
    mem_segment = str(read) + str(write) + format(min(row_addr, 511), '09b')  # Limit to 9 bits
    ...
    for instr in self.instructions:
        ...
        if op_type == self.instruction_types['PROG']:
            ...
            if read_bit == 1 and write_bit == 0:
                ...
                # According to ISA, when both read and write are high, read takes priority
...
def get_instruction_stats(self):
    ...
    stats = {
        ...
        'memory_reads': sum(1 for instr in self.instructions if instr[14:16] == '10' or instr[14:16] == '11'),
